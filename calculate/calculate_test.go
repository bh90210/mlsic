...
var linearScaleTestCases = map[string]struct {
	value    float64
	rMinMax  MinMax
	tMinMax  MinMax
	expected float64
...
func TestLinearScale(t *testing.T) {
	...
	for name, tc := range linearScaleTestCases {
		t.Run(name, func(t *testing.T) {
			result := LinearScale(tc.value, tc.rMinMax, tc.tMinMax)

			a.Equal(tc.isNaN, math.IsNaN(result))
...
var sineGenerationTestCases = map[string]struct {
	opts     SineOptions
	format   *audio.Format
	...
	"zero sine options": {
		...
		expected: audio.PCMBuffer{Format: &audio.Format{NumChannels: 1, SampleRate: 44100}, DataType: 0x4, SourceBitDepth: 0x20},
	},
	"simple": {
		opts: SineOptions{
			Freq: 440.,
...
func TestSineGeneration(t *testing.T) {
	...
	for name, tc := range sineGenerationTestCases {
		t.Run(name, func(t *testing.T) {
